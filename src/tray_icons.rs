...
/// Decode an embedded PNG to raw RGBA plus its actual dimensions, so swapped
/// assets can't silently mismatch a hardcoded size. macOS scales the tray
/// image down to menu-bar height, so a large source stays crisp on retina
/// displays.
#[cfg(feature = "tray")]
